...
*Setup instructions for a gitian build of Coinaaa using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Coinaaa
...
- [Building Coinaaa](#building-coinaaa)
...
Clone the git repositories for coinaaa and gitian,

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/coinaaa/coinaaa
```
...
In [doc/release-process.md](release-process.md) in the coinaaa repository under 'Fetch and build inputs'.
...
To build Coinaaa (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the coinaaa repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/coinaaa/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/coinaaa/coinaaa
...
For example:
```bash
URL=https://github.com/laanwj/coinaaa.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit coinaaa=${COMMIT} --url coinaaa=${URL} ../coinaaa/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit coinaaa=${COMMIT} --url coinaaa=${URL} ../coinaaa/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/coinaaa-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/coinaaa-build.assert
```
...
[coinaaa/gitian.sigs](https://github.com/coinaaa/gitian.sigs/) repository, or
