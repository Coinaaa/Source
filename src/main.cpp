...
#if defined(NDEBUG)
# error "Coinaaa cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Coinaaa Signed Message:\n";
                        ...
                        bool* pfMissingInputs, bool fRejectInsaneFee)
{
...

    {
        ...
        int64_t txMinFee = GetMinFee(tx, nSize, GMF_RELAY, GetAdjustedTime() < MINER_FEE_HARD_FORK);
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos)
{
    ...
    // Check the header
    if (Params().HashGenesisBlock() != block.GetHash() && !CheckProofOfWork(block.GetHash(), block.nBits))
...
int64_t GetBlockValue(int nHeight, int64_t nFees, unsigned int nTime) {
	// Before MINER_FEE_HARD_FORK Coinaaa had a 100% premine and no block fees.
	// Afterwards, all fees are collected
	if (nTime < MINER_FEE_HARD_FORK)
		return nHeight == 1 ? MAX_MONEY : 0;
	return nFees;
}

static const int64_t nTargetSpacing = 284; // 4min 44sec
static const int64_t nTargetTimespan = 30*nTargetSpacing; 
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    ...
    // Adapted from Peercoin's EMA algorithm.
    // Copyright (c) 2011-2013 PPCoin Developers
    ...
    static CBigNum bnLimit = Params().ProofOfWorkLimit();
    if (bnNew > bnLimit)
        bnNew = bnLimit;
...
bool ConnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (block.vtx[0].GetValueOut() > GetBlockValue(pindex->nHeight, nFees, pindex->nTime))
        return state.DoS(100,
                         error("ConnectBlock() : coinbase pays too much (actual=%d vs limit=%d)",
                               block.vtx[0].GetValueOut(), GetBlockValue(pindex->nHeight, nFees, pindex->nTime)),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // Check proof of work matches claimed amount
    if (fCheckPOW && block.hashPrevBlock != 0 && !CheckProofOfWork(block.GetHash(), block.nBits))
...
bool AcceptBlock(CBlock& block, CValidationState& state, CDiskBlockPos* dbp)
{
	...
	if (!block.CheckBlockSignature())
            return state.DoS(100, error("AcceptBlock() : bad block signature"),
...
// Following two functions derived from peercoin source
// Copyright (c) 2011-2013 PPCoin Developers

bool CBlock::SignBlock(const CKeyStore& keystore){

	CKey key;
	CPubKey centralKey = CPubKey(Params().CentralKey());

	if (!keystore.GetKey(Hash160(Params().CentralKey()), key))
		return false;

	if (key.GetPubKey() != centralKey)
		return false;

	return key.Sign(GetHash(), signature);

}

bool CBlock::CheckBlockSignature() const {

	CPubKey key = CPubKey(Params().CentralKey());
